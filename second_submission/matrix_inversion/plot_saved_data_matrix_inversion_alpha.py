...
def _inset_metadata_labels(fig_npz: Dict[str, np.ndarray]) -> tuple[str, str]:
    """Return ``(title, spectral_ratio_label)`` for the speedup inset.

    New fixed-alpha runs save ``alpha_inset`` and ``k_inset_list``.  In that
    case the inset uses K(d), so the title and spectral-ratio label must not
    refer to a fixed K.  Older saved data did not contain these keys, so we
    keep the old fixed-K labels as a backward-compatible fallback.
    """
    has_alpha = "alpha_inset" in fig_npz
    has_k_inset = "k_inset_list" in fig_npz

    if has_alpha:
        alpha = float(np.asarray(fig_npz["alpha_inset"]).ravel()[0])
        title = rf"Speedup, $\alpha={_format_float_for_math(alpha)}$"
        ratio_label = r"$\lambda_{K(d)+1}/\lambda_1$"
        return title, ratio_label

    if has_k_inset:
        k_vals = np.asarray(fig_npz["k_inset_list"]).astype(int).ravel()
        unique_k = np.unique(k_vals)
        if unique_k.size == 1:
            k = int(unique_k[0])
            title = rf"Speedup, $K={k}$"
            ratio_label = rf"$\lambda_{{{k + 1}}}/\lambda_1$"
        else:
            title = r"Speedup, $K=K(d)$"
            ratio_label = r"$\lambda_{K(d)+1}/\lambda_1$"
        return title, ratio_label

    k_speed = int(np.asarray(fig_npz.get("k_speed", 0)).ravel()[0])
    title = rf"Speedup, $K={k_speed}$"
    ratio_label = rf"$\lambda_{{{k_speed + 1}}}/\lambda_1$"
    return title, ratio_label
...
def plot_speedup_inset(
    ...
    *,
    ...
    inset_title, ratio_label = _inset_metadata_labels(fig_npz)
    ...
    axins.set_title(inset_title, fontsize=22)
...
def replot_fixed(fig_npz: Dict[str, np.ndarray]) -> plt.Figure:
    ...
    plot_speedup_inset(axins_fix, fig_npz, legend_anchor_y=0.8, clip_ylim=False)
