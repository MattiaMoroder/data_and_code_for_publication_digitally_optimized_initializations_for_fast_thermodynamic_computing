...
Usage:
  python3 plot_saved_data_determinant_alpha.py saved_data/ens=wishart-fixed__d=50-200-n3__K=5__alphainset=0.05__burn=eps0.001__tau=2__seed=0__hash=87154ea1 --plot_d 100 \
      --speedup_eps_list_wishart 0.1,0.05 \
      --speedup_eps_list_fixed 0.001,0.0005
"""
...
def _format_float_for_math(x: float) -> str:
    """Compact float formatting for math labels."""
    return f"{float(x):g}"


def _speedup_inset_title(
    *,
    mpemba_K: int,
    alpha_inset: Optional[float] = None,
    k_inset_list: Optional[np.ndarray] = None,
) -> str:
    """Return the title for the speedup inset.

    New fixed-alpha determinant runs save ``alpha_inset`` and ``k_inset_list``.
    In that case the inset data correspond to K(d), not to the fixed K used in
    the main panel.  Older saved data did not contain these keys, so we keep the
    original fixed-K label as a backward-compatible fallback.
    """
    if alpha_inset is not None and np.isfinite(alpha_inset):
        return rf"Speedup ($\alpha={_format_float_for_math(alpha_inset)}$)"

    if k_inset_list is not None:
        k_vals = np.asarray(k_inset_list).astype(int).ravel()
        if k_vals.size > 0:
            unique_k = np.unique(k_vals)
            if unique_k.size == 1:
                return rf"Speedup ($K={int(unique_k[0])}$)"
            return r"Speedup ($K=K(d)$)"

    return rf"Speedup ($K={mpemba_K}$)"
...
def _add_speedup_inset(
    ...
    """Add the speedup inset.

    The main panel may still show a fixed ``mpemba_K``.  If fixed-alpha inset
    arrays are present in the saved .npz file, this function should be called
    with those arrays and the corresponding ``alpha_inset``/``k_inset_list``
    metadata so the title reflects the actual inset data.
    """
...
def plot_from_npz(
    ...
    alpha_inset = (
        float(np.asarray(data["alpha_inset"]).ravel()[0])
        if "alpha_inset" in data
        else None
    )
    k_inset_list = (
        np.asarray(data["k_inset_list"], dtype=int)
        if "k_inset_list" in data
        else None
    )
    ...
    speedup_tburnK = tburnK
    try:
        if error_type == "rel":
            speedup_err0 = np.asarray(data["rel_eps0"], dtype=float)
            # New fixed-alpha runs save separate K(d) arrays for the inset.
            # Prefer them when available; otherwise fall back to the old fixed-K data.
            keyK = "rel_epsK_inset" if "rel_epsK_inset" in data else "rel_epsK"
            speedup_errK = np.asarray(data[keyK], dtype=float)
        else:
            key0 = "abs_eps0" if "abs_eps0" in data else "eps0"
            if "abs_epsK_inset" in data:
                keyK = "abs_epsK_inset"
            elif "epsK_inset" in data:
                keyK = "epsK_inset"
            elif "abs_epsK" in data:
                keyK = "abs_epsK"
            else:
                keyK = "epsK"
        ...
        if "tburnK_inset" in data and keyK.endswith("_inset"):
            speedup_tburnK = np.asarray(data["tburnK_inset"], dtype=float)
    except KeyError:
