...
- The forward burn-in time is then set by the next-slowest mode lambda_{K+1}
  instead of lambda_1.
- In convergence plots the main panels keep the user-specified fixed K, while
  the dimension-scaling insets can use K(d)=round(alpha_inset*d).
...
FAST-MAC EDITS:
- fixed-spectrum matrices are generated as diagonal matrices, because for A2=aI the protocol depends only on eigenvalues;
- the fast eigenbasis drive uses exact OU updates for each time step, so dt is no longer clamped by the largest eigenvalue;
- --skip_single avoids the preliminary sanity run before long convergence sweeps;
- --no_plots can save only npz data during long runs.
...
def make_J_fixed(d: int, alpha_min: float, alpha_max: float, rng: np.random.Generator) -> np.ndarray:
    ...
    # FAST-MAC: for the protocols used in this script, A2 is a scalar identity
    # and all dynamics/work statistics are evaluated in the eigenbasis of A1.
    # Therefore the Haar eigenvectors do not affect the results; only the
    # spectrum matters. Returning a diagonal matrix avoids an expensive QR +
    # dense reconstruction for large d.
    _ = rng  # kept for API compatibility/reproducibility of surrounding seed flow
...
def _mpemba_K_from_alpha(alpha: float, d: int) -> int:
    """Return K(d)=round(alpha*d), clipped to 0 <= K <= d-1.

    The half-up rounding convention matches the covariance script and avoids
    Python's banker's rounding for values such as alpha*d = 2.5.
    """
    if d <= 0:
        raise ValueError("d must be positive")
    return int(np.clip(int(math.floor(float(alpha) * int(d) + 0.5)), 0, int(d) - 1))
...
def simulate_burnin_and_work(
    ...
    *,
    ...
    for k in range(n_steps):
        ...
        # FAST-MAC: exact OU update for frozen lam_t over this time step.
        # This is more stable than Euler--Maruyama and allows larger dt.
        exp_step = np.exp(-lam_t * dt)
        var_step = (1.0 - np.exp(-2.0 * lam_t * dt)) / (beta * lam_t)
        y = y * exp_step + np.sqrt(var_step) * rng.normal(size=d)
...
def simulate_burnin_and_work_batch(
    ...
    *,
    ...
    for k in range(n_steps):
        ...
        # FAST-MAC: exact OU update for frozen lam_t over this time step.
        # This removes the stiff-mode Euler stability bottleneck at large d.
        exp_step = np.exp(-lam_t * dt)  # (d,)
        var_step = (1.0 - np.exp(-2.0 * lam_t * dt)) / (beta * lam_t)  # (d,)
        y = y * exp_step[None, :] + rng.normal(size=(n_traj, d)) * np.sqrt(var_step)[None, :]
...
def _safe_dt(dt: float, lam_max: float) -> float:
    """
    FAST-MAC: keep the user-requested dt.

    The fast path below now uses an exact Ornstein--Uhlenbeck update over each
    piecewise-constant time step, so the old Euler stability restriction
    dt * lambda_max <= 0.5 is no longer needed for the standard A1 <-> aI
    protocols used by this script. The argument lam_max is kept for API
    compatibility.
    """
    _ = lam_max
    return float(dt)
...
def run_convergence_study(
    ...
    *,
    ...
    mpemba_K_by_d: Optional[List[int]] = None,
    ...
    Returns:
      ...
      abs_epsK: (nD, nN, nA) Mpemba-K absolute logdet errors (all inf if no K is requested)
      rel_eps0: (nD, nN, nA) baseline relative logdet errors
      rel_epsK: (nD, nN, nA) Mpemba-K relative logdet errors (all inf if no K is requested)
    ...
    If ``mpemba_K_by_d`` is provided, it gives the requested K separately for each
    dimension. This is used only for the fixed-alpha inset; the usual fixed-K
    main-panel call leaves it as ``None``.
    """
    Ds = np.array(list(map(int, d_list)), dtype=int)
    Ns = np.array(list(map(int, n_list)), dtype=int)

    if mpemba_K_by_d is not None:
        if len(mpemba_K_by_d) != len(Ds):
            raise ValueError("mpemba_K_by_d must have the same length as d_list")
        K_by_d = np.array(list(map(int, mpemba_K_by_d)), dtype=int)
        # Treat an explicit per-d K list as requested inset data even if some
        # entries round to K=0; those entries are filled with the baseline data
        # and therefore give S=1 for that dimension.
        use_mpemba = True
    else:
        K_by_d = None
        use_mpemba = int(mpemba_K) > 0
    ...
    for i_d, d in enumerate(Ds):
        for j in range(n_A):
            ...
            K_requested = int(K_by_d[i_d]) if K_by_d is not None else int(mpemba_K)
            K = _effective_mpemba_K(K_requested, d, context="convergence study") if K_requested > 0 else 0

            if use_mpemba and K == 0:
                tburnK[i_d, j] = tburn0[i_d, j]
            elif use_mpemba and K > 0:
                lam_eff = float(eigvals[K])  # lambda_{K+1}
                if proto.burnin_mode == "mult":
            ...
            for i_n, N in enumerate(Ns):
                ...
                if estimator == "jarzynski":
                    ...
                    if use_mpemba:
                        if K == 0:
                            abs_epsK[i_d, i_n, j] = abs_eps0[i_d, i_n, j]
                            rel_epsK[i_d, i_n, j] = rel_eps0[i_d, i_n, j]
                        elif worksK_all is not None:
                            ...
                            logdetK = estimate_logdet_via_jarzynski(A1, n_traj=N, params=paramsK, mpemba_K=K)[0]
                            abs_epsK[i_d, i_n, j], rel_epsK[i_d, i_n, j] = logdet_errors(logdet_exact, logdetK)
                    ...
                    if use_mpemba:
                        if K == 0:
                            abs_epsK[i_d, i_n, j] = abs_eps0[i_d, i_n, j]
                            rel_epsK[i_d, i_n, j] = rel_eps0[i_d, i_n, j]
                        elif worksK_f_all is not None and worksK_r_all is not None:
                            deltaFK = _solve_bar_deltaF(worksK_f_all[:N], worksK_r_all[:N], paramsK.beta)
                            logdetK = logdet_A2 - 2.0 * paramsK.beta * deltaFK
                            abs_epsK[i_d, i_n, j], rel_epsK[i_d, i_n, j] = logdet_errors(logdet_exact, logdetK)
                        else:
                            logdetK = estimate_logdet_via_crooks(A1, n_traj_fwd=N, n_traj_rev=N, params=paramsK, mpemba_K_fwd=K)[0]
                            abs_epsK[i_d, i_n, j], rel_epsK[i_d, i_n, j] = logdet_errors(logdet_exact, logdetK)
                else:
...
def plot_convergence(
    ...
    errK_inset: Optional[np.ndarray] = None,
    tburnK_inset: Optional[np.ndarray] = None,
    alpha_inset: Optional[float] = None,
    k_inset_list: Optional[np.ndarray] = None,
) -> None:
    """
    ...
    # ---- Inset: determinant speedup S_eps(d) at fixed target error eps ----
    # The main panel uses the fixed K requested by --mpemba_K.  The inset can
    # instead use K(d)=round(alpha_inset*d), supplied through errK_inset/tburnK_inset.
    inset_errK = errK_inset if errK_inset is not None else errK
    inset_tburnK = tburnK_inset if tburnK_inset is not None else tburnK
    have_inset_data = np.any(np.isfinite(inset_tburnK))
    if have_inset_data and speedup_eps_list:
        ...
        for eps_target in speedup_eps_list:
            ...
            for i_d, d in enumerate(Ds):
                ...
                TK = _compute_T_epsilon(Ns, inset_errK[i_d, :, :], inset_tburnK[i_d, :], tau, eps_target, stat=speedup_stat, interp=speedup_interp)
        ...
        if alpha_inset is not None:
            axins.set_title(fr"Speedup ($\alpha={alpha_inset:g}$)", fontsize=8)
        else:
...
def main() -> None:
    ...
    ap.add_argument("--alpha_inset", type=float, default=-1.0,
                    help=("Digital fraction alpha = K/d used only in the dimension-scaling insets. "
                          "If <= 0, use alpha = mpemba_K / max(d_list)."))
    ...
    ap.add_argument("--skip_single", action="store_true",
                    help="Skip the preliminary single-run sanity estimate before convergence sweeps.")
    ap.add_argument("--no_plots", action="store_true",
                    help="Do not make PNG plots; useful for long runs where only --save_npz data are needed.")
    ...
    if not args.skip_single:
        ...
        print("[info] skipped preliminary single-run sanity estimate (--skip_single).")
    ...
    if args.convergence:
        ...
        k_inset_list: Optional[np.ndarray] = None
        if args.mpemba_K > 0:
            alpha_tmp = float(args.alpha_inset)
            if alpha_tmp <= 0.0:
                if len(d_list) == 0:
                    raise ValueError("d_list cannot be empty when using the default alpha_inset.")
                alpha_tmp = float(args.mpemba_K) / float(max(d_list))
            if not (0.0 < alpha_tmp < 1.0):
                raise ValueError("alpha_inset must satisfy 0 < alpha_inset < 1, or be <= 0 to use mpemba_K/max(d_list).")
            alpha_inset = alpha_tmp
            k_inset_list = np.array([_mpemba_K_from_alpha(alpha_inset, int(d)) for d in d_list], dtype=int)
        ...
        if args.save_npz:
            ...
            ad = dict(vars(args))
            ad["alpha_inset_effective"] = alpha_inset
            ad["k_inset_list_effective"] = (k_inset_list.tolist() if k_inset_list is not None else None)
            ...
            run_id = "__".join([
                ...
                d_tag,
                f"K={args.mpemba_K}",
                f"alphainset={(alpha_inset if alpha_inset is not None else 'NA')}",
                burn_tag,
                f"tau={args.tau:g}",
                f"seed={args.seed}",
                f"hash={param_hash}",
        ...
        for ens in ensembles:
            ...
            base_seed_ens = args.seed + 2024 + (0 if ens == "wishart" else 9999)
            Ns, Ds, abs_eps0, abs_epsK, rel_eps0, rel_epsK, tburn0, tburnK = run_convergence_study(
                ...
                base_seed=base_seed_ens,
                proto=proto,
                mpemba_K=args.mpemba_K,
                estimator=args.estimator,
            )

            # Additional convergence data used only for the inset, with K(d)=round(alpha*d).
            # The main panel above remains fixed-K.
            abs_epsK_inset = None
            rel_epsK_inset = None
            tburnK_inset = None
            if k_inset_list is not None and np.any(k_inset_list != int(args.mpemba_K)):
                _, _, _, abs_epsK_inset, _, rel_epsK_inset, _, tburnK_inset = run_convergence_study(
                    d_list=d_list,
                    n_list=n_list,
                    ensemble=ens,
                    n_A=nA,
                    ...
                    mpemba_K=args.mpemba_K,
                    estimator=args.estimator,
                    mpemba_K_by_d=list(map(int, k_inset_list)),
                )
            elif k_inset_list is not None:
                abs_epsK_inset = abs_epsK
                rel_epsK_inset = rel_epsK
                tburnK_inset = tburnK
            ...
            if args.save_npz:
                ...
                np.savez_compressed(
                    ...
                    tburn0=tburn0,
                    tburnK=tburnK,
                    abs_epsK_inset=abs_epsK_inset if abs_epsK_inset is not None else np.array([], dtype=float),
                    rel_epsK_inset=rel_epsK_inset if rel_epsK_inset is not None else np.array([], dtype=float),
                    tburnK_inset=tburnK_inset if tburnK_inset is not None else np.array([], dtype=float),
                    k_inset_list=k_inset_list if k_inset_list is not None else np.array([], dtype=int),
                    alpha_inset=float(alpha_inset) if alpha_inset is not None else np.nan,
                    tau=float(proto.tau),
            ...
            if not args.no_plots:
                plot_convergence(
                    ...
                    errK_inset=abs_epsK_inset,
                    tburnK_inset=tburnK_inset,
                    alpha_inset=alpha_inset,
                    k_inset_list=k_inset_list,
                )
